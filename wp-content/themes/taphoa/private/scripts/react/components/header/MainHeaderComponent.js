        ...
        return(
            <header>
                <div className={`main-header container ${open_class}`}>
                    <div id='logo'>
                        <Link to="/" onClick={()=> this._setPageTitle(title_seo)}>
                        ...
                        <ul className='nav-menus'>
                            {
                                menus && menus.map((item, index) => {
                                    return(
                                        <li key={uuidv4()}>
                                            <Link to={item.url} onClick={()=> this._setPageTitle(item.title_seo)}>{item.label}</Link>
                                            { item.subs && item.subs.length > 0 &&
                                                <ul className='sub-items'>
                                                    {
                                                        item.subs.map((subItem, subIndex) => 
                                                            <li key={uuidv4()}>
                                                                <Link onClick={()=> this._setPageTitle(subItem.title_seo)} to={subItem.url}>{subItem.label}</Link>
                                                            </li>
                                                        )
                                                    }
                                                </ul>
                                    ...
                                    menus && menus.map((item, index) => {
                                        return(
                                            <li key={uuidv4()} className={ item.subs && item.subs.length > 0 ? 'items have-sub' : 'items' }>
                                                <Link 
                                                    onClick={()=> this._setPageTitle(item.title_seo)}
                                                ...
                                                { item.subs && item.subs.length > 0 &&
                                                    <ul className='sub-items'>
                                                        { item.subs && item.subs.length > 0 && 
                                                            <li key={uuidv4()}>
                                                                <Link onClick={()=> this._setPageTitle(item.title_seo)} to={item.url}>{item.label}</Link>
                                                            </li> }
                                                        {
                                                            item.subs.map((subItem, subIndex) => 
                                                                <li key={uuidv4()}>
                ...
                <div className={`hamburger-menu ${open_class}`} onClick={()=> this.setState({ open_class: open_class == '' ? 'open' : '' })}><div className='line'></div></div>
            </header>
        )
    }
}
